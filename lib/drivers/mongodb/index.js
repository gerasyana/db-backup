...
class MongoDBDriver extends DriverBase {
    ...
    doBackup() {
        ...
        return doBackupPromise
            ...
            .catch(err => `Error while backing up database.\n ${err}`);
    }
    ...
    doRestore() {
        ...
        return doRestorePromise
            ...
            .catch(err => `Error while restoring database.\n ${err}`);
    }
    ...
    async _backupCollection(collection, fileName, backupOptions) {
        ...
        return new Promise((resolve, reject) => {
            ...
            readStream.
                on('error', error => {
                    spinner.stop();
                    reject(`Error while backing up ${collectionName}. ${error.message}`);
                }).
                on('end', () => {
                    spinner.stop();
                    resolve();
    ...
    async _restoreCollection(collection, filePath, dataFormat) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            readStream.on('error', error => {
                spinner.stop();
                reject(`Error while reading ${collection.collectionName}. ${error.message}`);
            });
